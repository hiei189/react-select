...
var instructionsAriaMessage = function(event) {
  ...
  switch (event) {
   case "menu":
    return "Utiliza flecha arriba y flecha abajo para seleccionar una opción".concat(isDisabled ? "" : ", presiona enter para seleccionar la opción en foco", ", presiona Escape para salir del menú, presiona Tab para seleccionar la opción y salir del menú.");

   case "input":
    return "".concat(label || "Select", " está seleccionado ").concat(isSearchable ? ", escribe para mejorar la lista" : "", ", presiona flecha abajo para abrir el menú, ").concat(isMulti ? " presiona flecha a la izquierda para dar foco a las opciones seleccionadas" : "");
  ...
  if (value) switch (event) {
   ...
   case "pop-value":
   case "remove-value":
    return "Opción ".concat(value, ", deseleccionada.");

   case "select-option":
    return "Opción ".concat(value, isDisabled ? " está deshabilitada. Selecciona otra opción." : ", seleccionada.");
  }
  ...
  return "Opción ".concat(getOptionLabel(focusedValue), " ahora en foco, ").concat(selectValue.indexOf(focusedValue) + 1, " de ").concat(selectValue.length, ".");
  ...
  return "Opción ".concat(getOptionLabel(focusedOption), " ahora en foco").concat(focusedOption.isDisabled ? " deshabilitada" : "", ", ").concat(options.indexOf(focusedOption) + 1, " de ").concat(options.length, ".");
}, resultsAriaMessage = function(_ref3) {
  var inputValue = _ref3.inputValue, screenReaderMessage = _ref3.screenReaderMessage;
  return "".concat(screenReaderMessage).concat(inputValue ? " para término de búsqueda " + inputValue : "", ".");
